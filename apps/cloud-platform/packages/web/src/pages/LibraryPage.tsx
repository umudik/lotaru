...
function clearPreviewUrl(urlRef: { current: string | null }): void {
...
export function LibraryPage() {
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      try {
        ...
        let folders = items;
        if (folders.length === 0) {
          const created = await createLibrary(activeSession, projectId, {
            title: "Files",
            id: "",
            description: "",
          });
          folders = await reloadLibraries();
          if (!active) return;
  ...
  const selectedDoc = useMemo(() => {
    ...
    return null;
  }, [libraryDetail, selectedDocumentId]);
  ...
  useEffect(() => {
    if (session === null || selectedDoc === null || !selectedDoc.hasFile) {
      clearPreviewUrl(previewUrlRef);
    ...
    void (async () => {
      try {
        ...
        const kind = documentPreviewKind(doc.mimeType, doc.originalName || doc.filename);
        if (kind === "text") {
          const body = await blob.text();
          if (!cancelled) {
            clearPreviewUrl(previewUrlRef);
            setPreview({ kind: "text", body });
          }
          return;
        }
  ...
  async function uploadFiles(files: FileList | File[]) {
    ...
    try {
      ...
      for (const file of list) {
        const created = await uploadLibraryDocument(session, projectId, selectedLibraryId, file);
        lastId = created.id;
      }
      await loadLibrary(selectedLibraryId);
      await reloadLibraries();
      if (lastId.length > 0) {
        setSelectedDocumentId(lastId);
        setSearchParams({ doc: lastId });
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid min-h-[70vh] grid-cols-1 gap-4 xl:grid-cols-[220px_minmax(0,1fr)_minmax(0,1.2fr)]">
        ...
        <article className="flex min-h-[20rem] flex-col rounded-xl border border-white/[0.06] bg-[#0c0c0c] p-4">
...
function DocumentReader(props: {
  ...
  selected: LibraryDocumentSummary | null;
}): React.JSX.Element {
  const preview = props.preview;
  if (props.selected === null) {
    return (
      <p className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
        Drop a file, then click it to read.
      </p>
    );
  }
  ...
  if (preview.kind === "text") {
    return (
      <pre className="min-h-0 flex-1 overflow-auto whitespace-pre-wrap break-words font-mono text-[13px] leading-6 text-foreground/90">
        {preview.body}
      </pre>
    );
  }
  if (preview.kind === "image") {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-auto">
        <img src={preview.url} alt={props.selected.title} className="max-h-full max-w-full rounded-lg" />
      </div>
    );
  }
  if (preview.kind === "pdf") {
    return (
      <iframe title={props.selected.title} src={preview.url} className="min-h-[28rem] w-full flex-1 rounded-lg bg-white" />
    );
  }
  ...
  if (preview.kind === "binary") {
    return (
      <p className="text-sm text-muted-foreground">
        This file type opens with Download. Text, markdown, PDF, and images read here.
      </p>
    );
  }
  return (
