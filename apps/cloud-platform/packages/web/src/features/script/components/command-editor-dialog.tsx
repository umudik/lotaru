...
function statusText(summary: { configured: boolean; lines: number }): string {
  if (!summary.configured) {
    return `Add ${runtimeLabel().toLowerCase()} script`;
  }
  if (summary.lines === 1) {
    return `1 line · ${runtimeLabel()}`;
  }
  return `${String(summary.lines)} lines · ${runtimeLabel()}`;
}
      ...
      >
        <DialogContent className="max-w-4xl w-[min(96vw,56rem)] h-[min(88vh,720px)] p-0 gap-0 flex flex-col overflow-hidden sm:rounded-xl">
          <DialogHeader className="px-5 pt-5 pb-3 shrink-0 border-b border-border/60">
            ...
            <DialogDescription>
              {runtimeLabel()} · runs in the project folder
            </DialogDescription>
          </DialogHeader>
